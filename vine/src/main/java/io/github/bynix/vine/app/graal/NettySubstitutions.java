    ...
    public static boolean isAlpnSupported(final SslProvider provider) {
        switch (provider) {
            ...
            case OPENSSL_REFCNT:
                return false;
            default:
                throw new Error("SslProvider unsupported on Vine " + provider);
        }
    }
}
...
final class Target_io_netty_handler_ssl_OpenSsl {
    ...
    private static Throwable UNAVAILABILITY_CAUSE = new RuntimeException("OpenSsl unsupported on Vine");
